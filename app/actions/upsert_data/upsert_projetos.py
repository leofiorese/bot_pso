...
CREATE TABLE IF NOT EXISTS `PROJETOS` (
    ...
    `CR_ID` INT,
    `RGR_HORAS_TRANSPORTE` VARCHAR(255),
    `RGR_HORAS_EXTRAS` VARCHAR(255),
    `RGR_CRIAR_ATIVIDADES` VARCHAR(255),
    `RGR_DELEGAR_ATIVIDADES` VARCHAR(255),
    `RGR_APONTAR_ATIVIDADES` VARCHAR(255),
    `RGR_INSTRUCOES_TRANSPORTE` VARCHAR(255),
    `VALOR` DECIMAL(18,2),
    ...
    `IND_IMPOSTOS_NA_TAXA_FAT` BOOLEAN,
    `COM_ID` INT,
    `COM_ID_HIS` INT,
    `INCLUIDO_EM` DATE,
    `ALTERADO_EM` DATE,
    ...
    `IND_APO_BLOQ` BOOLEAN,
    `IND_FAT_BLOQ` BOOLEAN,
    `IND_PLAN_BLOQ` BOOLEAN,
    `LIMITE_MINUTOS` INT,
    `VERSAO_CRONOGRAMA` INT,
    `IND_OS_HABILITADO` BOOLEAN,
    `IND_CALCULO_MANUAL` BOOLEAN,
    `IND_DT_PREV_ATUALIZADA` BOOLEAN,
    ...
    `UDF19` VARCHAR(255),
    `UDF20` VARCHAR(255),
    `IND_BILLIMATIC_FAT_AUTOMATICO` BOOLEAN,
    ...
    `AGILE_IND_APONTAMENTO` BOOLEAN,
    `AGILE_IND_PROJETO` BOOLEAN,
...
INSERT INTO PROJETOS (
    ...
    AGILE_IND_PROJETO, AGILE_PROJ_ID, CFG_EPT_APONTAMENTO, ESCOPO_VERSAO
) VALUES (
    ...
    %(AGILE_IND_PROJETO)s, %(AGILE_PROJ_ID)s, %(CFG_EPT_APONTAMENTO)s, %(ESCOPO_VERSAO)s
)
ON DUPLICATE KEY UPDATE
    ...
    ESCOPO_VERSAO = VALUES(ESCOPO_VERSAO);
"""
...
def clean_data(value, column_name):
    if column_name in [
        ...
        "INCLUIDO_EM","ALTERADO_EM","DT_ENCERRAMENTO","JIRA_DT_INICIO", "ESCOPO_VERSAO"
    ]:
        return convert_date(value)
    if column_name in [
        "IND_IMPOSTOS_NA_TAXA_FAT","IND_HR_FAT_ATIV_OS","IND_HORAS_FATURAVEIS","IND_APO_BLOQ",
        "IND_FAT_BLOQ","IND_PLAN_BLOQ","IND_OS_HABILITADO","IND_CALCULO_MANUAL",
        "IND_DT_PREV_ATUALIZADA","IND_BILLIMATIC_PREV","IND_BILLIMATIC_FAT_AUTOMATICO",
        "AGILE_IND_APONTAMENTO","AGILE_IND_PROJETO"
    ]:
        if value == 'Y':
            return True 
        elif value == 'N':
