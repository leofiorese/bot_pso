import logging
import pandas as pd
from db.db import get_conn
from datetime import datetime
import os
from utils import arquivar_csv

TABLE_COLUMNS = [
    "MF_ID",
    "EMP_ID",
    "CLI_ID",
    "FORN_ID",
    "PROJ_ID",
    "CR_ID",
    "USU_ID",
    "TIPO",
    "SUBTIPO",
    "VALOR",
    "VALOR_COM_IMPOSTOS",
    "VALOR_SEM_RET",
    "VALOR_MULTA",
    "VALOR_JUROS",
    "VALOR_PAGO",
    "SITUACAO",
    "NOTA_FISCAL",
    "NOTA_FISCAL_ORD",
    "RPS",
    "DATA_REFERENCIA",
    "DATA_VENCIMENTO",
    "DATA_VENC_ORIG",
    "DT_PAGAMENTO",
    "DATA_EMISSAO",
    "RGR_ID",
    "OBSERVACAO",
    "DESCRICAO",
    "CODIGO_SERVICO",
    "CATEGORIA_ID_MYFINANCE",
    "CENTRO_RECEITA_ID_MYFINANCE",
    "CONTA_RECEBIMENTO_ID_MYFINANCE",
    "NFE_COD_VER",
    "NOTIFICACAO",
    "MYFINANCE_ID",
    "READONLY",
    "INCLUIDO_EM",
    "ALTERADO_EM",
    "AUX",
    "FAT_ID",
    "CONTA_COBRANCA_ID_COBRATO",
    "TEMPLATE_ID_EMITES",
    "BILLIMATIC_ID",
    "UDF1",
    "UDF2",
    "UDF3",
    "UDF4",
    "UDF5",
    "UDF6",
    "UDF7",
    "UDF8",
    "UDF9",
    "UDF10",
    "ALIQ_APROX",
    "CONTA_RECEBIVEIS_ID_MYFINANCE",
    "MODELO_COBRANCA_ID_COBRATO",
    "OMIE_ID",
    "OMIE_SERVICO",
    "OMIE_COD_INTERNO",
    "OMIE_ID_ANTIGO",
    "OMIE_COD_INTERNO_ANTIGO",
    "OMIE_SERVICO_ANTIGO",
    "IND_FINAL",
    "C_IND_OP",
    "TP_OPER",
    "TP_ENTE_GOV",
    "IND_DEST",
    "TP_CHAVE_DFE",
    "X_TIPO_CHAVE_DFE",
    "CHAVE_DFE",
    "C_MUN_DOC_FISCAL",
    "N_DOC_FISCAL",
    "X_DOC_FISCAL",
    "N_DOC",
    "X_DOC",
    "CPF_FORN_DOC",
    "CNPJ_FORN_DOC",
    "NIF_FORN_DOC",
    "NAO_NIF",
    "NOME_FORN_DOC",
    "DT_EMISSAO_DOC_DED",
    "DT_COMPETENCIA_DOC_DED",
    "VL_BASE_IS",
    "VL_IS",
    "VL_BASE_CBS",
    "VL_CBS",
    "VL_BASE_IBS",
    "VL_IBS_ESTADUAL",
    "VL_IBS_MUNICIPAL",
    "VALOR_COM_IMPOSTOS_IVA",
    "ALIQ_IS",
    "ALIQ_CBS",
    "ALIQ_IBS_ESTADUAL",
    "ALIQ_IBS_MUNICIPAL",
    "TRIBUTACAO_RPS",
    "FONTE_CARGA_TRIBUT",
    "CODIGO_CEI",
    "MUNICIPIO_PRESTACAO",
    "NUMERO_ENCAPSULAMENTO",
    "EXIGIBILIDADE_SUSPENSA",
    "PGTO_PARCELADO_ANTECIPADO",
    "NCM",
    "NBS",
    "C_CLASS_TRIB",
    "C_CLASS_TRIB_REG",
    "TP_REE_REP_RES",
    "DESC_REE_REP_RES",
    "VL_REE_REP_RES",
    "C_LOC_PRESTACAO",
    "SERIE",
    "DISCRIMINACAO",
    "INSCRICAO_PRESTADOR"
]

CREATE_TABLE_SQL = """
CREATE TABLE IF NOT EXISTS `FATURAMENTO` (
    `MF_ID` INT,
    `EMP_ID` INT,
    `CLI_ID` INT,
    `FORN_ID` INT,
    `PROJ_ID` INT,
    `CR_ID` INT,
    `USU_ID` INT,
    `TIPO` INT,
    `SUBTIPO` INT,
    `VALOR` DECIMAL(18,2),
    `VALOR_COM_IMPOSTOS` DECIMAL(18,2),
    `VALOR_SEM_RET` DECIMAL(18,2),
    `VALOR_MULTA` DECIMAL(18,2),
    `VALOR_JUROS` DECIMAL(18,2),
    `VALOR_PAGO` DECIMAL(18,2),
    `SITUACAO` VARCHAR(100),
    `NOTA_FISCAL` VARCHAR(100),
    `NOTA_FISCAL_ORD` VARCHAR(100),
    `RPS` VARCHAR(100),
    `DATA_REFERENCIA` DATE,
    `DATA_VENCIMENTO` DATE,
    `DATA_VENC_ORIG` DATE,
    `DT_PAGAMENTO` DATE,
    `DATA_EMISSAO` DATE,
    `RGR_ID` INT,
    `OBSERVACAO` TEXT,
    `DESCRICAO` TEXT,
    `CODIGO_SERVICO` VARCHAR(100),
    `CATEGORIA_ID_MYFINANCE` INT,
    `CENTRO_RECEITA_ID_MYFINANCE` INT,
    `CONTA_RECEBIMENTO_ID_MYFINANCE` INT,
    `NFE_COD_VER` VARCHAR(50),
    `NOTIFICACAO` VARCHAR(255),
    `MYFINANCE_ID` INT,
    `READONLY` VARCHAR(10),
    `INCLUIDO_EM` DATE,
    `ALTERADO_EM` DATE,
    `AUX` DECIMAL(18,2),
    `FAT_ID` INT,
    `CONTA_COBRANCA_ID_COBRATO` INT,
    `TEMPLATE_ID_EMITES` INT,
    `BILLIMATIC_ID` INT,
    `UDF1` VARCHAR(255),
    `UDF2` VARCHAR(255),
    `UDF3` VARCHAR(255),
    `UDF4` VARCHAR(255),
    `UDF5` VARCHAR(255),
    `UDF6` VARCHAR(255),
    `UDF7` VARCHAR(255),
    `UDF8` VARCHAR(255),
    `UDF9` VARCHAR(255),
    `UDF10` VARCHAR(255),
    `ALIQ_APROX` DECIMAL(10,2),
    `CONTA_RECEBIVEIS_ID_MYFINANCE` INT,
    `MODELO_COBRANCA_ID_COBRATO` INT,
    `OMIE_ID` INT,
    `OMIE_SERVICO` VARCHAR(255),
    `OMIE_COD_INTERNO` VARCHAR(255),
    `OMIE_ID_ANTIGO` INT,
    `OMIE_COD_INTERNO_ANTIGO` VARCHAR(255),
    `OMIE_SERVICO_ANTIGO` VARCHAR(255),
    `IND_FINAL` INT,
    `C_IND_OP` INT,
    `TP_OPER` INT,
    `TP_ENTE_GOV` INT,
    `IND_DEST` INT,
    `TP_CHAVE_DFE` INT,
    `X_TIPO_CHAVE_DFE` VARCHAR(255),
    `CHAVE_DFE` VARCHAR(255),
    `C_MUN_DOC_FISCAL` VARCHAR(100),
    `N_DOC_FISCAL` VARCHAR(100),
    `X_DOC_FISCAL` VARCHAR(255),
    `N_DOC` VARCHAR(100),
    `X_DOC` VARCHAR(255),
    `CPF_FORN_DOC` VARCHAR(20),
    `CNPJ_FORN_DOC` VARCHAR(20),
    `NIF_FORN_DOC` VARCHAR(50),
    `NAO_NIF` VARCHAR(10),
    `NOME_FORN_DOC` VARCHAR(255),
    `DT_EMISSAO_DOC_DED` DATE,
    `DT_COMPETENCIA_DOC_DED` DATE,
    `VL_BASE_IS` DECIMAL(18,2),
    `VL_IS` DECIMAL(18,2),
    `VL_BASE_CBS` DECIMAL(18,2),
    `VL_CBS` DECIMAL(18,2),
    `VL_BASE_IBS` DECIMAL(18,2),
    `VL_IBS_ESTADUAL` DECIMAL(18,2),
    `VL_IBS_MUNICIPAL` DECIMAL(18,2),
    `VALOR_COM_IMPOSTOS_IVA` DECIMAL(18,2),
    `ALIQ_IS` DECIMAL(10,2),
    `ALIQ_CBS` DECIMAL(10,2),
    `ALIQ_IBS_ESTADUAL` DECIMAL(10,2),
    `ALIQ_IBS_MUNICIPAL` DECIMAL(10,2),
    `TRIBUTACAO_RPS` VARCHAR(100),
    `FONTE_CARGA_TRIBUT` VARCHAR(100),
    `CODIGO_CEI` VARCHAR(50),
    `MUNICIPIO_PRESTACAO` VARCHAR(100),
    `NUMERO_ENCAPSULAMENTO` INT,
    `EXIGIBILIDADE_SUSPENSA` VARCHAR(10),
    `PGTO_PARCELADO_ANTECIPADO` VARCHAR(10),
    `NCM` VARCHAR(20),
    `NBS` VARCHAR(20),
    `C_CLASS_TRIB` INT,
    `C_CLASS_TRIB_REG` INT,
    `TP_REE_REP_RES` INT,
    `DESC_REE_REP_RES` VARCHAR(255),
    `VL_REE_REP_RES` DECIMAL(18,2),
    `C_LOC_PRESTACAO` INT,
    `SERIE` VARCHAR(20),
    `DISCRIMINACAO` TEXT,
    `INSCRICAO_PRESTADOR` VARCHAR(50),

    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    PRIMARY KEY (`MF_ID`)
)
ENGINE=InnoDB
DEFAULT CHARSET=utf8mb4
COLLATE=utf8mb4_unicode_ci;
"""

UPSERT_SQL = """
INSERT INTO FATURAMENTO (
    MF_ID, EMP_ID, CLI_ID, FORN_ID, PROJ_ID, CR_ID, USU_ID, TIPO, SUBTIPO,
    VALOR, VALOR_COM_IMPOSTOS, VALOR_SEM_RET, VALOR_MULTA, VALOR_JUROS, VALOR_PAGO,
    SITUACAO, NOTA_FISCAL, NOTA_FISCAL_ORD, RPS, DATA_REFERENCIA, DATA_VENCIMENTO, DATA_VENC_ORIG, DT_PAGAMENTO, DATA_EMISSAO,
    RGR_ID, OBSERVACAO, DESCRICAO, CODIGO_SERVICO, CATEGORIA_ID_MYFINANCE, CENTRO_RECEITA_ID_MYFINANCE,
    CONTA_RECEBIMENTO_ID_MYFINANCE, NFE_COD_VER, NOTIFICACAO, MYFINANCE_ID, READONLY, INCLUIDO_EM, ALTERADO_EM, AUX, FAT_ID,
    CONTA_COBRANCA_ID_COBRATO, TEMPLATE_ID_EMITES, BILLIMATIC_ID, UDF1, UDF2, UDF3, UDF4, UDF5, UDF6, UDF7, UDF8, UDF9, UDF10,
    ALIQ_APROX, CONTA_RECEBIVEIS_ID_MYFINANCE, MODELO_COBRANCA_ID_COBRATO, OMIE_ID, OMIE_SERVICO, OMIE_COD_INTERNO,
    OMIE_ID_ANTIGO, OMIE_COD_INTERNO_ANTIGO, OMIE_SERVICO_ANTIGO,
    IND_FINAL, C_IND_OP, TP_OPER, TP_ENTE_GOV, IND_DEST, TP_CHAVE_DFE, X_TIPO_CHAVE_DFE, CHAVE_DFE,
    C_MUN_DOC_FISCAL, N_DOC_FISCAL, X_DOC_FISCAL, N_DOC, X_DOC, CPF_FORN_DOC, CNPJ_FORN_DOC, NIF_FORN_DOC,
    NAO_NIF, NOME_FORN_DOC, DT_EMISSAO_DOC_DED, DT_COMPETENCIA_DOC_DED, VL_BASE_IS, VL_IS, VL_BASE_CBS, VL_CBS,
    VL_BASE_IBS, VL_IBS_ESTADUAL, VL_IBS_MUNICIPAL, VALOR_COM_IMPOSTOS_IVA,
    ALIQ_IS, ALIQ_CBS, ALIQ_IBS_ESTADUAL, ALIQ_IBS_MUNICIPAL, TRIBUTACAO_RPS, FONTE_CARGA_TRIBUT, CODIGO_CEI,
    MUNICIPIO_PRESTACAO, NUMERO_ENCAPSULAMENTO, EXIGIBILIDADE_SUSPENSA, PGTO_PARCELADO_ANTECIPADO, NCM, NBS,
    C_CLASS_TRIB, C_CLASS_TRIB_REG, TP_REE_REP_RES, DESC_REE_REP_RES, VL_REE_REP_RES, C_LOC_PRESTACAO,
    SERIE, DISCRIMINACAO, INSCRICAO_PRESTADOR
) VALUES (
    %(MF_ID)s, %(EMP_ID)s, %(CLI_ID)s, %(FORN_ID)s, %(PROJ_ID)s, %(CR_ID)s, %(USU_ID)s, %(TIPO)s, %(SUBTIPO)s,
    %(VALOR)s, %(VALOR_COM_IMPOSTOS)s, %(VALOR_SEM_RET)s, %(VALOR_MULTA)s, %(VALOR_JUROS)s, %(VALOR_PAGO)s,
    %(SITUACAO)s, %(NOTA_FISCAL)s, %(NOTA_FISCAL_ORD)s, %(RPS)s, %(DATA_REFERENCIA)s, %(DATA_VENCIMENTO)s, %(DATA_VENC_ORIG)s, %(DT_PAGAMENTO)s, %(DATA_EMISSAO)s,
    %(RGR_ID)s, %(OBSERVACAO)s, %(DESCRICAO)s, %(CODIGO_SERVICO)s, %(CATEGORIA_ID_MYFINANCE)s, %(CENTRO_RECEITA_ID_MYFINANCE)s,
    %(CONTA_RECEBIMENTO_ID_MYFINANCE)s, %(NFE_COD_VER)s, %(NOTIFICACAO)s, %(MYFINANCE_ID)s, %(READONLY)s, %(INCLUIDO_EM)s, %(ALTERADO_EM)s, %(AUX)s, %(FAT_ID)s,
    %(CONTA_COBRANCA_ID_COBRATO)s, %(TEMPLATE_ID_EMITES)s, %(BILLIMATIC_ID)s, %(UDF1)s, %(UDF2)s, %(UDF3)s, %(UDF4)s, %(UDF5)s, %(UDF6)s, %(UDF7)s, %(UDF8)s, %(UDF9)s, %(UDF10)s,
    %(ALIQ_APROX)s, %(CONTA_RECEBIVEIS_ID_MYFINANCE)s, %(MODELO_COBRANCA_ID_COBRATO)s, %(OMIE_ID)s, %(OMIE_SERVICO)s, %(OMIE_COD_INTERNO)s,
    %(OMIE_ID_ANTIGO)s, %(OMIE_COD_INTERNO_ANTIGO)s, %(OMIE_SERVICO_ANTIGO)s,
    %(IND_FINAL)s, %(C_IND_OP)s, %(TP_OPER)s, %(TP_ENTE_GOV)s, %(IND_DEST)s, %(TP_CHAVE_DFE)s, %(X_TIPO_CHAVE_DFE)s, %(CHAVE_DFE)s,
    %(C_MUN_DOC_FISCAL)s, %(N_DOC_FISCAL)s, %(X_DOC_FISCAL)s, %(N_DOC)s, %(X_DOC)s, %(CPF_FORN_DOC)s, %(CNPJ_FORN_DOC)s, %(NIF_FORN_DOC)s,
    %(NAO_NIF)s, %(NOME_FORN_DOC)s, %(DT_EMISSAO_DOC_DED)s, %(DT_COMPETENCIA_DOC_DED)s, %(VL_BASE_IS)s, %(VL_IS)s, %(VL_BASE_CBS)s, %(VL_CBS)s,
    %(VL_BASE_IBS)s, %(VL_IBS_ESTADUAL)s, %(VL_IBS_MUNICIPAL)s, %(VALOR_COM_IMPOSTOS_IVA)s,
    %(ALIQ_IS)s, %(ALIQ_CBS)s, %(ALIQ_IBS_ESTADUAL)s, %(ALIQ_IBS_MUNICIPAL)s, %(TRIBUTACAO_RPS)s, %(FONTE_CARGA_TRIBUT)s, %(CODIGO_CEI)s,
    %(MUNICIPIO_PRESTACAO)s, %(NUMERO_ENCAPSULAMENTO)s, %(EXIGIBILIDADE_SUSPENSA)s, %(PGTO_PARCELADO_ANTECIPADO)s, %(NCM)s, %(NBS)s,
    %(C_CLASS_TRIB)s, %(C_CLASS_TRIB_REG)s, %(TP_REE_REP_RES)s, %(DESC_REE_REP_RES)s, %(VL_REE_REP_RES)s, %(C_LOC_PRESTACAO)s,
    %(SERIE)s, %(DISCRIMINACAO)s, %(INSCRICAO_PRESTADOR)s
)
ON DUPLICATE KEY UPDATE
    EMP_ID = VALUES(EMP_ID),
    CLI_ID = VALUES(CLI_ID),
    FORN_ID = VALUES(FORN_ID),
    PROJ_ID = VALUES(PROJ_ID),
    CR_ID = VALUES(CR_ID),
    USU_ID = VALUES(USU_ID),
    TIPO = VALUES(TIPO),
    SUBTIPO = VALUES(SUBTIPO),
    VALOR = VALUES(VALOR),
    VALOR_COM_IMPOSTOS = VALUES(VALOR_COM_IMPOSTOS),
    VALOR_SEM_RET = VALUES(VALOR_SEM_RET),
    VALOR_MULTA = VALUES(VALOR_MULTA),
    VALOR_JUROS = VALUES(VALOR_JUROS),
    VALOR_PAGO = VALUES(VALOR_PAGO),
    SITUACAO = VALUES(SITUACAO),
    NOTA_FISCAL = VALUES(NOTA_FISCAL),
    NOTA_FISCAL_ORD = VALUES(NOTA_FISCAL_ORD),
    RPS = VALUES(RPS),
    DATA_REFERENCIA = VALUES(DATA_REFERENCIA),
    DATA_VENCIMENTO = VALUES(DATA_VENCIMENTO),
    DATA_VENC_ORIG = VALUES(DATA_VENC_ORIG),
    DT_PAGAMENTO = VALUES(DT_PAGAMENTO),
    DATA_EMISSAO = VALUES(DATA_EMISSAO),
    RGR_ID = VALUES(RGR_ID),
    OBSERVACAO = VALUES(OBSERVACAO),
    DESCRICAO = VALUES(DESCRICAO),
    CODIGO_SERVICO = VALUES(CODIGO_SERVICO),
    CATEGORIA_ID_MYFINANCE = VALUES(CATEGORIA_ID_MYFINANCE),
    CENTRO_RECEITA_ID_MYFINANCE = VALUES(CENTRO_RECEITA_ID_MYFINANCE),
    CONTA_RECEBIMENTO_ID_MYFINANCE = VALUES(CONTA_RECEBIMENTO_ID_MYFINANCE),
    NFE_COD_VER = VALUES(NFE_COD_VER),
    NOTIFICACAO = VALUES(NOTIFICACAO),
    MYFINANCE_ID = VALUES(MYFINANCE_ID),
    READONLY = VALUES(READONLY),
    INCLUIDO_EM = VALUES(INCLUIDO_EM),
    ALTERADO_EM = VALUES(ALTERADO_EM),
    AUX = VALUES(AUX),
    FAT_ID = VALUES(FAT_ID),
    CONTA_COBRANCA_ID_COBRATO = VALUES(CONTA_COBRANCA_ID_COBRATO),
    TEMPLATE_ID_EMITES = VALUES(TEMPLATE_ID_EMITES),
    BILLIMATIC_ID = VALUES(BILLIMATIC_ID),
    UDF1 = VALUES(UDF1),
    UDF2 = VALUES(UDF2),
    UDF3 = VALUES(UDF3),
    UDF4 = VALUES(UDF4),
    UDF5 = VALUES(UDF5),
    UDF6 = VALUES(UDF6),
    UDF7 = VALUES(UDF7),
    UDF8 = VALUES(UDF8),
    UDF9 = VALUES(UDF9),
    UDF10 = VALUES(UDF10),
    ALIQ_APROX = VALUES(ALIQ_APROX),
    CONTA_RECEBIVEIS_ID_MYFINANCE = VALUES(CONTA_RECEBIVEIS_ID_MYFINANCE),
    MODELO_COBRANCA_ID_COBRATO = VALUES(MODELO_COBRANCA_ID_COBRATO),
    OMIE_ID = VALUES(OMIE_ID),
    OMIE_SERVICO = VALUES(OMIE_SERVICO),
    OMIE_COD_INTERNO = VALUES(OMIE_COD_INTERNO),
    OMIE_ID_ANTIGO = VALUES(OMIE_ID_ANTIGO),
    OMIE_COD_INTERNO_ANTIGO = VALUES(OMIE_COD_INTERNO_ANTIGO),
    OMIE_SERVICO_ANTIGO = VALUES(OMIE_SERVICO_ANTIGO),
    IND_FINAL = VALUES(IND_FINAL),
    C_IND_OP = VALUES(C_IND_OP),
    TP_OPER = VALUES(TP_OPER),
    TP_ENTE_GOV = VALUES(TP_ENTE_GOV),
    IND_DEST = VALUES(IND_DEST),
    TP_CHAVE_DFE = VALUES(TP_CHAVE_DFE),
    X_TIPO_CHAVE_DFE = VALUES(X_TIPO_CHAVE_DFE),
    CHAVE_DFE = VALUES(CHAVE_DFE),
    C_MUN_DOC_FISCAL = VALUES(C_MUN_DOC_FISCAL),
    N_DOC_FISCAL = VALUES(N_DOC_FISCAL),
    X_DOC_FISCAL = VALUES(X_DOC_FISCAL),
    N_DOC = VALUES(N_DOC),
    X_DOC = VALUES(X_DOC),
    CPF_FORN_DOC = VALUES(CPF_FORN_DOC),
    CNPJ_FORN_DOC = VALUES(CNPJ_FORN_DOC),
    NIF_FORN_DOC = VALUES(NIF_FORN_DOC),
    NAO_NIF = VALUES(NAO_NIF),
    NOME_FORN_DOC = VALUES(NOME_FORN_DOC),
    DT_EMISSAO_DOC_DED = VALUES(DT_EMISSAO_DOC_DED),
    DT_COMPETENCIA_DOC_DED = VALUES(DT_COMPETENCIA_DOC_DED),
    VL_BASE_IS = VALUES(VL_BASE_IS),
    VL_IS = VALUES(VL_IS),
    VL_BASE_CBS = VALUES(VL_BASE_CBS),
    VL_CBS = VALUES(VL_CBS),
    VL_BASE_IBS = VALUES(VL_BASE_IBS),
    VL_IBS_ESTADUAL = VALUES(VL_IBS_ESTADUAL),
    VL_IBS_MUNICIPAL = VALUES(VL_IBS_MUNICIPAL),
    VALOR_COM_IMPOSTOS_IVA = VALUES(VALOR_COM_IMPOSTOS_IVA),
    ALIQ_IS = VALUES(ALIQ_IS),
    ALIQ_CBS = VALUES(ALIQ_CBS),
    ALIQ_IBS_ESTADUAL = VALUES(ALIQ_IBS_ESTADUAL),
    ALIQ_IBS_MUNICIPAL = VALUES(ALIQ_IBS_MUNICIPAL),
    TRIBUTACAO_RPS = VALUES(TRIBUTACAO_RPS),
    FONTE_CARGA_TRIBUT = VALUES(FONTE_CARGA_TRIBUT),
    CODIGO_CEI = VALUES(CODIGO_CEI),
    MUNICIPIO_PRESTACAO = VALUES(MUNICIPIO_PRESTACAO),
    NUMERO_ENCAPSULAMENTO = VALUES(NUMERO_ENCAPSULAMENTO),
    EXIGIBILIDADE_SUSPENSA = VALUES(EXIGIBILIDADE_SUSPENSA),
    PGTO_PARCELADO_ANTECIPADO = VALUES(PGTO_PARCELADO_ANTECIPADO),
    NCM = VALUES(NCM),
    NBS = VALUES(NBS),
    C_CLASS_TRIB = VALUES(C_CLASS_TRIB),
    C_CLASS_TRIB_REG = VALUES(C_CLASS_TRIB_REG),
    TP_REE_REP_RES = VALUES(TP_REE_REP_RES),
    DESC_REE_REP_RES = VALUES(DESC_REE_REP_RES),
    VL_REE_REP_RES = VALUES(VL_REE_REP_RES),
    C_LOC_PRESTACAO = VALUES(C_LOC_PRESTACAO),
    SERIE = VALUES(SERIE),
    DISCRIMINACAO = VALUES(DISCRIMINACAO),
    INSCRICAO_PRESTADOR = VALUES(INSCRICAO_PRESTADOR);
"""

def create_table(cursor, table_name):
    try:
        cursor.execute(CREATE_TABLE_SQL)
        logging.info(f"Tabela {table_name} criada/verificada.")
    except Exception as e:
        logging.error(f"Erro ao criar/verificar a tabela: {e}")
        raise

def convert_date(value):
    if isinstance(value, str):
        try:
            return datetime.strptime(value, '%d/%m/%Y').strftime('%Y-%m-%d')
        except ValueError:
            return None  
    return value

def clean_data(value, column_name):
    if column_name in [
        "DATA_REFERENCIA",
        "DATA_VENCIMENTO",
        "DATA_VENC_ORIG",
        "DT_PAGAMENTO",
        "DATA_EMISSAO",
        "INCLUIDO_EM",
        "ALTERADO_EM",
        "DT_EMISSAO_DOC_DED",
        "DT_COMPETENCIA_DOC_DED"
    ]:
        return convert_date(value)
    if column_name in [
        "IND_FINAL",
        "IND_DEST"
    ]:
        if value == 'Y':
            return True 
        elif value == 'N':
            return False
    if pd.isna(value) or value == "" or value is None or pd.isnull(value):
        return None
    return value

def upsert_data(df: pd.DataFrame, table_name: str, csv_file_path: str):
    conn = None
    cursor = None
    try:
        conn = get_conn()
        cursor = conn.cursor()
        create_table(cursor, table_name)
        for col in df.columns:
            df[col] = df[col].apply(lambda x: clean_data(x, col))
        for _, row in df.iterrows():
            data_tuple = row.to_dict()
            for key, value in data_tuple.items():
                if isinstance(value, float) and pd.isna(value):
                    data_tuple[key] = None 
            cursor.execute(UPSERT_SQL, data_tuple)
        conn.commit()
        arquivar_csv(csv_file_path, table_name)
    except Exception as e:
        if conn:
            conn.rollback()
        raise
    finally:
        if cursor:
            cursor.close()
        if conn:
            conn.close()